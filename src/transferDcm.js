...
function getDcmDetail(unit8s) {
  ...
  while (cur < unit8s.length) {
    ...
    const VL = utils.computeValueLength(VR);
...
function getMeta(unit8s) {
  ...
  if (identifier !== 'DICM') {
    throw new Error('The picture is not DICM.');
  } else {
...
function parsePixelData(meta) {
  if (meta['00280004'].includes('MONOCHROME2')) {
    ...
    for (let i = 0; i < dcmData.length; i += 2) {
      let hu = parseInt(rescaleSlope, 10) * utils.litteEndianCompute(dcmData.slice(i, i + 2)) + parseInt(rescaleIntercept, 10);
  ...
  throw new Error('Currently only supports Dcm with MONOCHROME2.');
}
